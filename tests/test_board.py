...
def test_matched_cells_detects_horizontal_and_vertical_runs() -> None:
    board = np.array([[1, 1, 1], [2, 3, 4], [2, 5, 6]])
    assert matched_cells(board) == {(0, 0), (0, 1), (0, 2)}


def test_find_best_move_creates_match() -> None:
    board = np.array(
        [
            [1, 2, 1, 4],
            [3, 1, 5, 6],
...
def test_cell_center_maps_grid_to_screen() -> None:
